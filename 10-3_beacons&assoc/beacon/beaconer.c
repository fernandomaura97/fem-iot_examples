...
PROCESS_THREAD(nullnet_example_process, ev, data)
{
  ...
  printf("beacon type: %d\t", beacon_type);
  uint8_t beacon_n =0;
  ...
  nullnet_buf = (uint8_t *)&beaconbuf; //initialize buffer
  ...
  while(1) {
    ...
    for (i=0; i<3; i++) { 
      ...
      //sensor_type_byte = 0b11111111; //uncomment to poll every nodeid
    ...
    for (i2=1; i2<9; i2++) {
      if (sensor_type_byte & 0x01) { //If bit is set, poll node
