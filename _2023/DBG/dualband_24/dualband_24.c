...
PROCESS(reboot_prc,"reboot process");
...
void serial_in(){ // Implementa la lògica a la cadena de caràcters que ha entrat al UART. node_db_24
    ...
    if (strncmp(header, "BO", sizeof("B0")) == 0){ // B0 indicarà al db_24 que el missatge és beacon
      ...
      else if(strncmp(header, "RST", sizeof("RST")) == 0){ // P0 indicarà al db_24 que el missatge és un poll

        process_start(&reboot_prc,"RESET");
...
{
  static struct etimer rst_3;
  PROCESS_BEGIN();
  while(1) {
    
    //PROCESS_WAIT_EVENT_UNTIL(ev == PROCESS_EVENT_POLL);

    printf("Rebooting in delta 3\n");
