...
static uint8_t death_counter = 0;
...
PROCESS_THREAD(coordinator_process, ev,data)
{
    ...
    {   
        ...
        if(!rxflags.id1 && !rxflags.id2 &&!rxflags.id3 &&!rxflags.id4 &&!rxflags.id5 &&!rxflags.id6 &&!rxflags.id7 &&!rxflags.id8 )
        {
            
            death_counter++; 
            LOG_ERR("Every node dropped, +1 to RST: %d",death_counter);
            if(death_counter >=3)
            {
                process_start(&reboot_process, "rst");
            }
