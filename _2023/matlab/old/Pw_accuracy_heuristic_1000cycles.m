...
[data_heur,time,channel_info] = thingSpeakRead(readChannelID,'Fields',TemperatureFieldID,'ReadKey',readAPIKey ,DateRange=[datetime(2022,7,5,17,47,00),datetime(2022,7,6,10,25,00)]);

%%% TO TEST
%[sampling_values,ttimes,ci] = thingSpeakRead(1762776,'Fields',[1,2,3,4], 'NumPoints',500);
%%% END TO TEST
...
t1_clean = rmoutliers(t1_h);
t2_clean = rmoutliers(t2_h);
t3_clean = rmoutliers(t2_h);
t4_clean = rmoutliers(t4_h);

plot(time(index_id1),t1_h)
hold on;
plot(time(index_id2),t2_h)
hold on;
plot(time(index_id3),t3_h)
hold on;
plot(time(index_id4),t4_h)
%%subplots, etc
